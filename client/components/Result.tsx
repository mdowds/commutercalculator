...
export default class Result extends React.Component<ResultProps, {}> {
    ...
    makeDetails(result: JourneyResult) {
        const travelcardDiv = result.seasonTickets.travelcard ? this.travelcardDescription(result.seasonTickets.travelcard) : null;
        ...
        const seasonTicketsDescription = this.seasonTicketsHasValues(result.seasonTickets) ?
    ...
    render() {
        ...
        const details = this.props.isSelected ? <div style={{margin: '10px 0px 0px 30px', fontSize: '75%'}}>{this.makeDetails(this.props.result)}</div> : null;

        const icon = this.props.isSelected ? "img/chevron-up.svg" : "img/chevron-down.svg";

        return (
            <div style={divStyle} onClick={() => {this.props.onSelectResult(this.props.result)}}>
                <div>

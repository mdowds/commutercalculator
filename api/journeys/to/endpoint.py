...
from api.types import JourneysToArgs, JourneyResult, OutputDict, JourneyPrice
...
@curried
def _join_data(prices: Tuple[JourneyPrice, ...], time: JourneyTime) -> JourneyResult:
    price = find(lambda p: p.origin == time.origin, prices)
    return JourneyResult(time.origin, time.time, price.travelcard)
